...
export default function OrderList() {
  ...
  const { superAdminBuyers } = useAppSelector((state) => {
    const raw = state.pharmacistBuyerList.superAdminBuyers;
    return {
      superAdminBuyers: raw as unknown as PharmacyBuyerResponse | null,
    };
  });
  ...
  const selectedBuyerMobile = (() => {
    if (!selectedBuyer || !superAdminBuyers?.data) return "";

    const buyer = superAdminBuyers.data.find((b) => b.id === selectedBuyer);
  ...
  useEffect(() => {
    let baseData: PharmacistOrder[] = Array.isArray(orders) ? orders : [];

    // 🟢 Buyer selected → filter by mobile number
    if (selectedBuyer && selectedBuyerMobile) {
      baseData = baseData.filter(
        (o) =>
          String(o.buyerNumber ?? "").trim() ===
          String(selectedBuyerMobile).trim()
      );
    }
  ...
  }, [orders, selectedBuyer, selectedBuyerMobile, searchTerm, orderType]);
  ...
  const handleBuyerChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedBuyer(value ? Number(value) : "");
  };
